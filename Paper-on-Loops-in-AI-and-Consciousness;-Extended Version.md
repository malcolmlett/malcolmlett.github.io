...
For that reason, a third form of model exists, whereby a secondary model is constructed to predict the behaviours of the former......(..tbd...) the secondary model may for example capture a subtly different aspect of the underlying system modelled by the former, or it may do so with lower resolution. This is at heart of Higher Order Thought Theory (citation), and recent theories based on it such as Hierarchical Active Inference (citation), and World Models (citation).
