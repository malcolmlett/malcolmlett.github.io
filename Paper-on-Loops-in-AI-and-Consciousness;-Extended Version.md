...
There is an additional, simpler, way of looking at all this. Consider the function `f(x) = 3 - 2x`. This function is, for example, effective at predicting the action required to regulate back towards a target state of `3` by doubling the input signal and comparing to that target state. However it is impossible for the function to be executed in such a way as to produce a representation of itself. There is no single `x` that we can feed into `f(x)` in order to get as output the message "3 minus 2 times x". We could extend the function to a piecewise of the form:

             { 3 - 2x, if i = 0
    f(x,i) = {  
             { "3 minus 2 times x", if x = 1
             
However, this is really just two independent functions `f(x)` and `f(x)'`, where `f(x)` generates values and `f'(x)` generates messages, and we hope that the message generated by `f'(x)` corresponds to the equation executed by `f(x)`. 

An alternative, is that we have `f` issues a representational model, without executing it, and a separate function `g`, executes that. Thus, the full output is `g(f(x))`. Now, by separating the modelling and the execution into separate components, it is possible to have a single function that can be used both for generation of action, but also for inspection of the model without direct action generation.
